-- The assistant's reports, on delivered revenue.
--
-- `admin_chat_data` summed `o.total` and `l.quantity`, so an administrator
-- asking the assistant for revenue got a different number than the dashboard
-- beside it once any order was partly delivered. Same rule, same source: the
-- order-level figures come from `order_revenue` and every line-level one
-- multiplies by `delivered_quantity`.
--
-- The body below is the deployed function with only those expressions changed.

CREATE OR REPLACE FUNCTION public.admin_chat_data(p_report text, p_store_id text DEFAULT NULL::text, p_limit integer DEFAULT 10, p_months integer DEFAULT 6)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  if p_report = 'overview' then
    ...
    from (
      select
        ...
        coalesce((select sum(r.realized_total) from public.order_revenue r
          where r.store_id = s.id and r.realized), 0) as net_revenue,
    ...
    into v_result
    from (
      select r.status, count(*) as order_count, coalesce(sum(r.realized_total), 0) as recorded_total
    ...
    from (
      select
        ...
        sum(l.delivered_quantity) as units,
        count(distinct l.order_id) as order_count,
        coalesce(sum(l.line_price * l.delivered_quantity), 0) as revenue,
        coalesce(sum(coalesce(l.margin, 0) * l.delivered_quantity), 0) as profit
    ...
    from (
      select
        ...
        coalesce(sum(r.realized_total), 0) as net_revenue,
