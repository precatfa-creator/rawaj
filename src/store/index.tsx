...
const storeColumns = 'id,name,image,productCount:product_count,customerCount:customer_count,orderCount:order_count,totalProfit:total_profit,lastActivity:last_activity';
const zoneColumns = 'id,name,region,capital,areaKm2:area_km2,population,fee,deliveryTimeDays:delivery_time_days,active';
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const loadZones = async () => {
      const { data, error } = await supabase.from('delivery_zones').select(zoneColumns).order('population', { ascending: false });
